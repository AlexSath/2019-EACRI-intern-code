...
def compare_fingerprints(fingerprint_path_array, fingerprint_name_array, compare_dir):
    script_path = os.path.join(current_dir_path, '..', 'fingerprintScripts', 'compareDMFs.pl')
    ...
    num_comparisons = (fpt.ncr(len(fingerprint_path_array), 2) * 2) + len(fingerprint_path_array)
    for (p, n) in zip(fingerprint_path_array, fingerprint_name_array):
        for (p2, n2) in zip(fingerprint_path_array, fingerprint_name_array):
            print(f'Comparing {n} with {n2}... [{current_comp_number}/{num_comparisons}]') #so you know what is going on...
...
'''INCOMPLETE -- Creates comparisons based on a .csv file tumor_normal key.
